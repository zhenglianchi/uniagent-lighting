...
# 黑盒：沙箱内 claude-code 访问 Gateway——direct-URL（定稿方案）：
#   GATEWAY_PORT 固定 Gateway 监听端口（verl_gateway_fixed_port + gateway_fixed_port 补丁）
#   + 安全组放行该端口 + CLAUDE_GATEWAY_PUBLIC_HOST=训练机公网 IP
