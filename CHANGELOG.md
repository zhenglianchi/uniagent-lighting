...
- **`tencent_stop_all_instances.py` 新增 `--destroy` 模式**：对每个实例发 E2B
  kill（DELETE /sandboxes/{id}）请求并报告结果。实测结论：**腾讯云 Agent Runtime
  没有删除实例的 API**——控制面 ags 只有 Start/Stop/Pause/Resume/Update；E2B 兼容
  端点的 kill 也被映射为"停止"，对已 STOPPED 实例返回 `UnsupportedOperation`
  （400）。STOPPED 实例无法手动销毁，靠腾讯侧"超时删除/自动空闲回收"清理
- **计费结论（官方文档）**：Agent 沙箱按量计费，计算资源（CPU 0.000081 元/核/秒、
  内存 0.000025 元/GiB/秒）按**实际运行时长**计费——实例停止后计算资源停止计费；
  系统盘 0.0021 元/GiB/小时但每实例 15 GiB 免费额度内（沙箱默认容量内）0 费用；
  暂停/停止期间存储费用内测期暂不收取 → STOPPED 实例不会产生费用，且过期后
  自动回收
