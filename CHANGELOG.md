...
- **docs/简历亮点.md 口径修正（用户拍板：本地补丁不算亮点）**：
  - 删除 `verl#7014`（LoRA merge 梯度）与 `vllm#30059`（EAGLE-3 logprobs）
    两个本地补丁的"上游 RL 正确性 bug"亮点表述——仅为本地仓库补丁、
    未提官方 PR，不构成开源贡献亮点
  - 亮点一改为"Agent RL 训练链路从零打通与排障"，保留 Gateway 工具解析
    错误杀会话的排障故事（5.3% 解析失败率 / 38% 会话受影响 / JSON 修复重试）
...
- **文档全面重写，平台化为主线**：
  - README：详细项目介绍（特性/架构图/训练结果对比表/快速开始/裸机安装/
    训练模式/评测/文档索引/已知问题），参考主流开源项目格式；明确
    **两种运行形态**：平台化形态（主线，用户侧 agent → 云端 Gateway）与
    内部训练形态（训练机侧 runner，链路与 on-policy 本质一致）
  - architecture.md 重写：平台化分层架构（Agent/Gateway/沙箱/数据/训练/服务）
  - deployment.md：新增 §2.1「从裸机到全部训练」分步路线 + 验证清单，
    各章节统一平台化表述
  - vllm_access / 修改与补丁汇总 / 训练评测分析：黑盒等表述改为
    「经 Gateway + MCP 工具转发接入」
  - 简历亮点 / ROADMAP：双机全异步 + Mooncake 标记完成（83.23%）
...
- **文档统一"平台化训练结果"口径**：README / 训练评测分析统一声明 baseline、
  spec、黑盒全部训练结果均为**平台化训练产出**（平台化链路已验证可加载/继续
  这些权重）；README 补黑盒 25/25 完成 + 最终评测 80.75%（平台化通过率）
- 服务器清理：黑盒中间 checkpoint（step 6/15）删除、仅留最终 25，磁盘释放
  32G（23G → 55G 可用）；训练日志/轨迹/模型/数据/评估全部保留
- 代码同步：本地 / 服务器 / 远端一致（v0.45.x）

## v0.45.1（2026-08-13）

- **修正黑盒 vs 白盒对比数字**：训练 reward mean 黑盒 ~0.60 vs 白盒 0.589
  （基本持平，非此前误写的"高 15-20pp"）；真正差异是起点（黑盒 step1 0.57
  vs 白盒 0.29）与最终评测（黑盒 80.75% vs 白盒 83.2%）；训练评测分析 §8.3、
  简历亮点第 3 点同步修正
...
- **新增 `docs/简历亮点.md`**：项目简介 + 5 个亮点（Agent RL 链路排障 /
  Rollout 优化 / 黑盒行为分析 / 平台化架构 / 双机全异步-待补实测）+ 包装口径。
  双机实测数据补后更新。
...
- **目录整洁**：删除已废弃的单机双卡冒烟脚本 `run_grpo_dualgpu_ucloud.sh`
  （2×24GB 方案已弃，多机路径取代；git 历史可恢复）；清理本地
  `__pycache__` 残留（17 个 .pyc，含已删源文件的缓存）
...
- **README 全面更新**：新增「训练模式一览」（冒烟 / 单机 LoRA / agentic / 全样本
  HumanEvalFix / 投机推理 / 多机，含启动示例）、「我们做了什么」进度时间线（全样本
  baseline、gateway 修复、EAGLE-3 投机推理 +40%、评测结果、日志存档）、部署教程补全
  （speculator 下载、补丁清单、脚本软链清单、腾讯沙箱排坑）

## v0.33.4（2026-08-10）

- **沙箱清理脚本恢复原版**：`tencent_stop_all_instances.py` 回退 `--destroy` 模式
  （v0.33.3 曾新增），恢复为训练/评测跑通时使用的原版（仅列出 + 停止运行中实例，
  释放 CPU 配额）
- **计费结论备忘（官方文档）**：Agent 沙箱按量计费，计算资源（CPU/内存）按实际
  运行时长计费——实例停止后停止计费；系统盘每实例 15 GiB 免费额度内 0 费用；
  STOPPED 实例不产生费用，过期后由腾讯侧"超时删除/自动空闲回收"清理
