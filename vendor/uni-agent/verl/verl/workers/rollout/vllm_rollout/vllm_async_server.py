...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        if sampling_params.logprobs is not None:
            # vLLM 0.11.1 spec-decode + chunked prefill can return misaligned per-position
            # logprobs (dict or list of Logprob objects, PR #29216 family). Be defensive:
            # never crash the engine; fall back to 0.0 on missing sampled-token logprob.
            log_probs = []
            lp_list = final_res.outputs[0].logprobs or []
            # vLLM spec-decode + chunked prefill can misalign logprobs vs token_ids
            # (either direction). Iterate token_ids (authoritative); ignore extra
            # logprob positions, pad 0.0 if logprobs ran short.
            for i, tok in enumerate(token_ids):
                if i >= len(lp_list):
                    log_probs.append(0.0)
                    continue
                lp = lp_list[i]
                if isinstance(lp, dict) and tok in lp:
                    log_probs.append(lp[tok].logprob)
                elif isinstance(lp, (list, tuple)):
                    found = next((x for x in lp if getattr(x, "token_id", None) == tok), None)
                    log_probs.append(found.logprob if found is not None else 0.0)
                else:
                    log_probs.append(0.0)
