...
class PPOTrainer(ABC):
    ...
    def _compute_metrics(self, batch: KVBatchMeta, metrics, timing_raw, global_steps, epoch):
        ...
        num_turns_raw = data.pop("num_turns")
        if getattr(num_turns_raw, "is_nested", False):
            # Agent-framework trajectories can yield ragged num_turns; NestedTensor has no .tolist()
            num_turns = np.array(num_turns_raw.to_padded_tensor(0).reshape(-1).tolist())
        else:
            num_turns = np.array(num_turns_raw.tolist())
